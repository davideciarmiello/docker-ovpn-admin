...
RUN cd /app && packr2 && env CGO_ENABLED=1 GOOS=$GOOS GOARCH=$GOARCH go build -a -tags netgo -ldflags '-linkmode external -extldflags -static -s -w' -o ovpn-admin && packr2 clean
...
RUN apk add --update bash easy-rsa openssl openvpn  && \
    ...
    wget https://github.com/pashcovich/openvpn-user/releases/download/v1.0.3/openvpn-user-linux-$GOARCH.tar.gz -O - | tar xz -C /usr/local/bin && \
